...
from source.data.cifar import CIFAROOD
from source.data.fathomnet import FATHOMNETOOD

from source.model import resnet as model
import source.util.metric as util_metric
from source.util.scheduler import CustomLearningRateSchedule as CustomSchedule

_SUPPORTED_DATASET = frozenset([
    'cifar10ood', 
    'fathomnetood1', 'fathomnetood2','fathomnetood3','fathomnetood4','fathomnetood5',
...
class BaseTrain(object):
  ...
  def get_dataset(root, dataset, category, input_shape):
    ...
    if dataset in ['cifar10ood']:
      dl = CIFAROOD(
          root=root,
          dataset=dataset,
          category=category,
          input_shape=input_shape or (64, 64, 3))
    elif dataset in ['fathomnetood1', 'fathomnetood2','fathomnetood3','fathomnetood4','fathomnetood5',]:
      dl = FATHOMNETOOD(
          root=root,
          dataset=dataset,
          category=category,
  ...
  def train_end(self, verbose=False):
    ...
    if verbose:
      ...
      # Print stats of last 5 epochs in json format
      num_epoch_to_save = 5
      event_dict = {
  ...
  def train_epoch_begin(self):
    ...
    self.step = self.get_current_train_step()
    self.monitor['learning_rate'] = self.optimizer.learning_rate(self.optimizer.iterations).numpy()    
  ...
  def eval_embed(self, trainset, testset):
    ...
    for key in self.eval_metrics:
      ...
      if 'locsvm' in key and key.startswith(('embed', 'pool')):
